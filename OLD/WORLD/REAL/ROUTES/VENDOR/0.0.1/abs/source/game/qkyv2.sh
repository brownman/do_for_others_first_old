...
##############################################################
# QUACKEY-V2: The "vulnerable" version of quackey.           #
# Experimental: Appends to save-file all possible            #
#               7+ letter anagrams.                          #
#                                                            #
...
{ # Save and display stats.
  ...
  savefile=qky.save.$$
  #                 ^^ PID of script
  echo "*** Experimental *** Quackey/Vulnerable" >> $savefile
  echo >> $savefile
  echo `date` >> $savefile
  ...
  echo "---------" >> $savefile
  echo >> $savefile
  echo "All possible 7+ letter anagrams from letterset:" >> $savefile
  echo >> $savefile

  echo  "${LS[@]}" | anagram | grep '.......' >> $savefile
  #                                  1234567
  # Uses "anagram" utility, which is part of "yawl" word list package.
  # Extraneous line appended: "Enter a LETTERSET to test ..."
...
# This script most certainly has bugs.
